...
function compute_weighted_triangles(n::Int64,
    ...
    for l in range(1, stop=s)
        ...
        println(a, " ", b, " ", length(neighbors(adj_list, a)), " ", length(neighbors(adj_list, b)))
        ...
        while c == b
            ...
            if trials > 500
                println(a_nbrs)
                println(nbr_weights[a] / nbr_z[a])
                println(nbr_prefix[a])
                println(sampled_c)
                println(us)
                println(uu)
                while true
            ...
            u = rand()
            modified_u = (u * nbr_z[a])
            modified_u -= degrees[b]*adj_list.weights[a,b]
            modified_u -= nbr_z[b]
            modified_u /= degrees[b]
            c = a_nbrs[searchsortedfirst(nbr_prefix[a], modified_u)]
            push!(sampled_c, c)
            push!(us, modified_u)
            push!(uu, u)
        end
        ...
        while cprime == a
            u = rand()
            modified_u = (u * nbr_z[b] - (adj_list.weights[a,c] + adj_list.weights[a,b]))
            cprime = b_nbrs[searchsortedfirst(nbr_prefix[b], modified_u)]
        end
